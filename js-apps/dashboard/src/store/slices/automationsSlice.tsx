...
// No global loading overlay: the builder autosaves on every canvas change
// (including a node drag), and a blocking full-screen spinner on each one would
// fight the very interaction that triggered it. The canvas reports the outcome
...
// Status transitions return no record, so the new status is written onto both
// the list row and the open record — the builder header reads the latter. Ids
// are compared as strings: the route supplies one, the API a number.
const applyStatus = (
  ...
  if (
    ...
    String(state.selectedAutomation.id) === String(id)
  ) {
    state.selectedAutomation.status = status;
  }
};
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(activateAutomation.fulfilled, (state, action) => {
        applyStatus(state, action.payload.id, 'active');
      })

      // Pause automation
      .addCase(pauseAutomation.fulfilled, (state, action) => {
        applyStatus(state, action.payload.id, 'paused');
      })
