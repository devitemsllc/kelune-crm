...
import { ProUpgradeNotice } from '../common/ProFeatureGate';
...
const AddStepDrawer = ({
  ...
  // 'condition' processor in Free), so the tab offers the upgrade teaser instead
  // of the config form — except on an existing condition step (e.g. after a
  // downgrade), which stays viewable and removable.
  ...
  const conditionLocked =
    activeTab === 'condition' && !proActive && editNode?.type !== 'condition';
  const hasEmailSteps = activeTab === 'action' && actionType === 'send_email';
  const inEmailWizard = hasEmailSteps && emailStep < EMAIL_STEPS.length - 1;
  ...
  return (
    <Drawer
      ...
      footer={
        // The action picker has nothing to save until a type is chosen, and a
        // locked Condition tab has nothing to save at all.
        isPickingAction || conditionLocked ? null : (
          <ModalFooter
            onOk={inEmailWizard ? handleEmailNext : () => form.submit()}
            okText={
              inEmailWizard
                ? __('Next', 'kelune-crm')
                : __('Save', 'kelune-crm')
    ...
    >
      {/* Enter advances the send_email wizard; on its last pane the panel's own
          hidden submit takes over, so this handler must not swallow the key. */}
      <div
        className="kelune-crm-cc-add-step"
        onKeyDown={inEmailWizard ? onEnterKey(handleEmailNext) : undefined}
      >
        ...
        {conditionLocked ? (
          <ProUpgradeNotice
            title={__('Conditions', 'kelune-crm')}
            description={__(
              'Upgrade to Kelune CRM Pro to branch an automation on tags, segments and email engagement.',
              'kelune-crm'
            )}
            padding="48px 0"
          />
