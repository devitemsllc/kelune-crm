...
const ImportModal = ({ visible, onClose, onSuccess }: ImportModalProps) => {
  ...
  const handleUpload = (file: RcFile) => {
    ...
    reader.onload = async (e) => {
      try {
        if (isExcel) {
          // Lazy-load the (heavy) xlsx parser only when an Excel file is picked.
  ...
  // Upload (step 0) has nothing to commit yet, so it shows no actions at all.
  const footer =
    currentStep === 1 ? (
      <ModalFooter
        onOk={handleImport}
      ...
      <ModalFooter onOk={handleFinish} okText={__('Done', 'kelune-crm')} />
    ) : null;
    ...
    >
      ...
      {currentStep === 0 && (
        <>
          <Alert
            message={__(
              'Upload a CSV or Excel file whose first row contains column headers. Contacts are matched by email — existing contacts are updated, new emails are created.',
              'kelune-crm'
            )}
            type="info"
          ...
          >
            ...
            <p className="kelune-crm-ant-upload-hint" style={{ marginTop: 0 }}>
              {__('CSV or Excel (.csv, .xlsx, .xls)', 'kelune-crm')}
            </p>
          </Dragger>
        </>
      )}
