...
// In-editor (iframe) content styling. TinyMCE renders the editing surface in an
// iframe, so nothing from the page cascades in — the typed content only matches
// the rest of the app if these rules restate it here.
const CONTENT_STYLE = `
  body {
...
// Prop-independent init options, under WordPress's TinyMCE 4.x option names.
...
// Rich-text email editor over WordPress core's bundled TinyMCE 4.x. Shaped as a
// value/onChange component for AntD Form.Item; uncontrolled internally, so
// callers reseed by changing this component's React `key`.
const RichTextEditor = ({
  ...
  // Keeps the events TinyMCE fires while initialising out of `onChange`.
  ...
  // Init reads these once; a ref keeps that read off a stale closure.
  ...
  useEffect(() => {
    ...
    tinymce.init({
      ...
      toolbar1: [
        ...
        .join(' '),
      // `setup` is the earliest v4 hands the instance over; holding it here
      // lets an unmount mid-init still tear the editor down.
      setup: (editor: WpEditor) => {
        ...
        // Native "Placeholders" menu button: inserts the chosen merge tag at
        // the cursor. Only registered when placeholders are supplied (the
        // toolbar entry is filtered out otherwise, above).
  ...
  // `kelune-crm-cc-editor-container` carries the TinyMCE chrome styling in base.css
  // (border, radius, transparent toolbar with #d9d9d9 dividers);
  // `wp-editor-container` is what WordPress's own editor styles key off.
  const finalEditor = (
    <div
      ref={wrapperRef}
  ...
  // so TinyMCE's iframe measures correctly while a Card spinner overlays it.
  // `display:none` would leave the raw <textarea> zero-sized or visible.
  return (
