...
/**
 * Label for a <Progress> showing a rate. Left to itself a bar swaps its label
 * for a success tick at 100%, so the best-performing row is the one whose
 * number disappears — every rate bar passes this as `format`.
 */
export const ratePercent = (percent?: number): string => `${percent ?? 0}%`;
