/**
 * Ant Design + dayjs locale loading, keyed off `window.kelunecrm.locale`
 * (WordPress `get_user_locale()`, e.g. `fr_FR`).
 *
 * Neither library names its packs the way WordPress does — antd uses
 * `language_REGION` (`fr_FR`), dayjs lowercase dashed codes (`pt-br`), and WP
 * codes like `bel`/`ckb`/`kmr` resolve to differently named packs. So each
 * library gets an explicit loader map plus a `wpTo*Alias` table, and candidates
 * are tried most-specific-first (`de_DE_formal` → `de_DE` → `de`) with the alias
 * applied at every level.
 *
 * Loaders use bare-specifier dynamic imports so Vite emits one lazy chunk per
 * locale and only the site's own locale is fetched. English needs no pack.
 *
 * To add a locale: add the import to the relevant loader map, and a `wpTo*Alias`
 * entry when the WP code differs from the pack name.
 */
...
export async function loadLocale(): Promise<Locale | undefined> {
  ...
  // `en_US` and bare `en` are the built-in defaults — skip them. Other English
  // variants still load their own pack (en_GB, en-au, … carry the correct
