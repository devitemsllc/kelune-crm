import type { CSSProperties } from 'react';

interface Tint {
  background: string;
  border: string;
}

/** Soft tinted info-card backgrounds, one hue per metric. */
export const INFO_CARD_TINTS = {
  blue: {
    background: 'linear-gradient(135deg, #eff6ff 0%, #f5f9ff 100%)',
    border: '#bae0ff',
  },
  green: {
    background: 'linear-gradient(135deg, #f0fdf4 0%, #f6fdf9 100%)',
    border: '#b7eb8f',
  },
  red: {
    background: 'linear-gradient(135deg, #fff1f0 0%, #fff7f6 100%)',
    border: '#ffccc7',
  },
  orange: {
    background: 'linear-gradient(135deg, #fff7ed 0%, #fffaf5 100%)',
    border: '#ffd591',
  },
  volcano: {
    background: 'linear-gradient(135deg, #fff2e8 0%, #fff8f4 100%)',
    border: '#ffbb96',
  },
  cyan: {
    background: 'linear-gradient(135deg, #e6fffb 0%, #f0fffd 100%)',
    border: '#87e8de',
  },
  purple: {
    background: 'linear-gradient(135deg, #faf5ff 0%, #fbf8ff 100%)',
    border: '#d3adf7',
  },
  magenta: {
    background: 'linear-gradient(135deg, #fff0f6 0%, #fff7fa 100%)',
    border: '#ffadd2',
  },
} satisfies Record<string, Tint>;

export type InfoCardTint = keyof typeof INFO_CARD_TINTS;

/** Card style for a tinted info card. */
export const tintedCardStyle = (tint: InfoCardTint): CSSProperties => ({
  background: INFO_CARD_TINTS[tint].background,
  border: `1px solid ${INFO_CARD_TINTS[tint].border}`,
  boxShadow: 'none',
});
