...
/**
 * Resolve the editor a template uses from its stored structure: `builder` when
 * the structure says so, or when it carries a block tree but no `mode` at all.
 * Anything else opens in Rich Text, mirroring the content editor's own
 * normalisation.
 */
