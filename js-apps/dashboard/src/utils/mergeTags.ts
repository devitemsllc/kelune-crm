...
// Shared by every footer editor (global settings and per-template custom
// footer) so both state the same requirement. A function, not a const, so `__()`
// resolves at render time once i18n is ready.
export const footerComplianceMessage = (): string =>
  __(
    'Include your business name, postal address and the {{unsubscribe_url}} merge tag — anti-spam law requires all three, and without them your emails are likely to be treated as spam.',
    'kelune-crm'
  );
