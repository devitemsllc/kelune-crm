...
/** A time-of-day component in an otherwise date-shaped string. */
const HAS_TIME = /\d{1,2}:\d{2}/;

/**
 * Calendar date shown exactly as its source states it, with no timezone
 * conversion — a license expiry or a birthday is the same day everywhere, and
 * shifting it by the viewer's offset moves it to the wrong date. Values that are
 * not dates at all (a license server's "No expiry", "Unlimited") pass through
 * verbatim.
 */
export const calendarFormat = (value: DateInput): string => {
  if (!value) {
    return '';
  }
  ...
  return HAS_TIME.test(raw)
    ? parsed.format('MMMM D, YYYY h:mm a')
