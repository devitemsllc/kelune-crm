...
const Campaigns = () => {
  ...
  // Identity, recipients and the send — the email body is authored on the
  // campaign builder route.
  const handleConfigure = (record: Campaign) => {
  ...
  // Activation is the send. The backend refuses an incomplete campaign (no
  // subject, body or recipients) and says which, so the message is passed
  // straight through instead of being second-guessed here.
  const handleActivate = async (record: Campaign) => {
    try {
      await dispatch(activateCampaign(record.id)).unwrap();
      message.success(
        campaignDisplayState({ ...record, status: CAMPAIGN_STATUS.ACTIVE }) ===
          'scheduled'
      ...
      message.error(
        getErrorMessage(error, __('Failed to activate campaign', 'kelune-crm'))
      );
    }
  };
  ...
  // Activating and pausing belong to the Status switch in its own column, so the
  // menu carries only the actions that have nowhere else to live.
  ...
  const allColumns: VisibleColumn[] = [
    {
      ...
      render: (_, record) => (
        <div>
          <Link
            to={`/campaigns/builder/${record.id}`}
    ...
    },
    {
      ...
      // The switch is both the readout and the send control: on = dispatch
      // permitted. Its label names the state the campaign is actually in, so a
      // scheduled and an in-flight campaign don't read the same. A sent campaign
      // is terminal, so the switch shows it without offering to flip it.
      render: (_status: string | undefined, record) => (
        <InlineSwitch
          inline
          style={{ marginBottom: 0 }}
          label={campaignStateLabel(record)}
          checked={isCampaignActive(record)}
          disabled={!canToggleCampaign(record)}
          onChange={(checked) =>
            checked ? handleActivate(record) : handlePause(record.id)
          }
        />
      ),
    },
  ...
  const columnOptions = [
    ...
    { key: 'scheduled_at', label: __('Scheduled', 'kelune-crm') },
      ...
      chips: [
        {
          ...
          label: campaignStatusLabel(filters.status),
    ...
    <div className="kelune-crm-cc-campaigns-container">
      ...
      <CampaignForm
        ...
        onCreated={(campaign) => {
          setConfigDrawerVisible(false);
          setEditingCampaign(null);
          navigate(`/campaigns/builder/${String(campaign.id)}`);
        }}
      />

      <ProUpgradeModal
        open={proUpgradeVisible}
        onClose={() => setProUpgradeVisible(false)}
        title={__('A/B Testing', 'kelune-crm')}
      />
