...
export interface Contact extends StrictBase {
  ...
  /**
   * Resolved server-side (Services/AvatarService): the stored avatar, else a
   * Gravatar while use_gravatar_service is on, else '' (render initials).
   * Resolved on the contact list only; other routes send the raw column.
   */
  avatar_url?: string;
...
/**
 * GET /email-logs/stats payload. Counts come back as strings, rates as
 * numbers. All optional, numerics widened to number|string.
 */
...
export interface CampaignVariant extends StrictBase {
  ...
  subject?: string;
  email_content?: string;
  /** Editor the content was written in: `builder` or `richtext`. */
  content_mode?: string;
  /** Editor state behind the content (block tree + chrome settings). */
  json_structure?: unknown;
  from_name?: string;
  sent_count?: number;
  open_count?: number;
...
/** Live A/B picture for one campaign: settings, the control, and the variants. */
export interface CampaignAbTest {
  enabled: boolean;
  sample_size: number;
  winner_metric: string;
  /** Hours the test runs before a winner is picked. Never zero. */
  winner_wait_hours: number;
  /**
   * UTC moment the automatic decision falls due. Null while the sample is still
   * sending, and when the sample is the whole audience.
   */
  winner_decision_at: string | null;
  /** Variants may only be changed while the campaign is still a draft. */
  editable: boolean;
  control: {
    percentage: number;
    sent_count: number;
  ...
  /** Decided winner: a variant id, 0 for the control, null while undecided. */
  winner_variant_id: number | null;
  remainder_queued: boolean;
}
...
/**
 * Cron Monitor (Settings → Cron Monitor), served by ToolsController. Mirrors
...
/**
 * The Pro license as the license server describes it. Every field is a
 * server-rendered string — `expire_date` / `support_end` carry human phrases
 * ("No expiry", "Unlimited") as often as dates, so they are displayed verbatim
 * and never parsed. `license_key` arrives masked.
 */
export interface LicenseData {
  ...
  msg?: string;
  renew_link?: string;
  expire_renew_link?: string;
  support_renew_link?: string;
}

/** GET /license, and the payload activate/deactivate resolve with. */
