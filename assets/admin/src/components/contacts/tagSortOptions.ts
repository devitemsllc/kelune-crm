...
// Sortable fields for Tags, mirroring the backend's whitelisted orderby columns
// (id, name, contact_count, created_at). `id` is the implicit default (newest
// first) and is intentionally not offered here. `contact_count` is a computed
// JOIN count in the repository, not the (unused) stored column.
export const SORT_OPTIONS: SortOption[] = [
  { value: 'name', label: __('Tag name', 'kelune-crm') },
...
// date (oldest/newest first) alongside the real timestamp.
export const CHRONOLOGICAL_FIELDS = ['id', 'created_at'];
export const NUMERIC_FIELDS: string[] = ['contact_count'];

// Default: `id` DESC (newest tags first). `id` is not a user-selectable field,
