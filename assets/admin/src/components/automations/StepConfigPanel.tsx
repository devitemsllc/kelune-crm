...
// Module scope on purpose: EmailContentEditor memoises off these names, so
// inline array literals would hand it a new identity on every render of this
// panel and remount the editor mid-edit.
const EMAIL_MODE_NAME = ['action_config', 'content_mode'];
const EMAIL_CONTENT_NAME = ['action_config', 'body'];
const EMAIL_STRUCTURE_NAME = ['action_config', 'json_structure'];
const EMAIL_TEMPLATE_ID_NAME = ['action_config', 'template_id'];
...
const normalizeStepData = (
  ...
  if (ac && typeof ac === 'object') {
    ...
    // Form.Item initialValue: a step saved in a given editor must reopen ON that
    // editor, not silently default and hide its design. Prefer the mode the
    // structure records; fall back for legacy steps that stored none.
    if (!nac.content_mode) {
      ...
      // removed html/text modes) reopens as Rich Text. A step with no structure
      // at all is a fresh/legacy raw body → Rich Text.
      nac.content_mode =
        stored === 'builder'
          ? 'builder'
          : stored
  ...
  // Trigger config ids also arrive as strings; coerce them so the Select
  // options (numeric values) match and render the label, not the raw id.
  const tc = next.trigger_config;
  if (tc && typeof tc === 'object') {
    const ntc = { ...(tc as Record<string, unknown>) };
    if (ntc.tag_id != null) ntc.tag_id = toNum(ntc.tag_id);
    if (ntc.list_id != null) ntc.list_id = toNum(ntc.list_id);
    if (ntc.segment_id != null) ntc.segment_id = toNum(ntc.segment_id);
    if (ntc.campaign_id != null) ntc.campaign_id = toNum(ntc.campaign_id);
    next.trigger_config = ntc;
  }
...
const StepConfigPanel = ({
  ...
  const triggerTypeValue = Form.useWatch('trigger_type', form) as
    | string
    | undefined;
  ...
  // One-shot load of every option list the panel offers. Defined inside the
  // effect because nothing else calls these, which keeps the dependency list
  // honest: `proActive` reads a global fixed for the session, so this runs once.
  useEffect(() => {
  ...
  // Trigger-specific config, mirroring the automation settings modal so the
  // canvas can show AND edit the actual enrolment target (tag/list/segment/
  // campaign/date). Stored under trigger_config and persisted to the automation
  // record (not the step) by AutomationCanvas.
        ...
        return (
          <>
            ...
            <div style={{ display: emailStep === 1 ? 'block' : 'none' }}>
              <EmailContentEditor
                form={form}
                modeName={EMAIL_MODE_NAME}
                contentName={EMAIL_CONTENT_NAME}
                structureName={EMAIL_STRUCTURE_NAME}
                templateIdName={EMAIL_TEMPLATE_ID_NAME}
            ...
            <>
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: __('Please select a trigger type', 'kelune-crm'),
                  },
                ]}
                tooltip={__(
                  'Sets what enrols contacts into this automation. Saved on the automation itself and mirrored here on the canvas.',
                  'kelune-crm'
                )}
              >
                <Select
                  placeholder={__('Select trigger type', 'kelune-crm')}
            ...
            <>
              ...
              >
                <Select
                  ...
                  // Segment conditions depend on Pro segments — hidden until Pro
                  // is active.
                  options={[
                    {
                      value: 'field_value',
                    ...
                    (o) => proActive || !PRO_CONDITION_TYPES.has(o.value)
                  )}
                />
              </Form.Item>
