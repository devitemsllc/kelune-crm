...
// A small Switch with a same-line, clickable label (support-genix
// docs-suggestions pattern, sized to match the app's inline switches). The
// label toggles the switch on click. Two modes:
...
const InlineSwitch = (props: InlineSwitchProps) => {
  const { label, tooltip, style, inline } = props;
  ...
  return (
    ...
    >
      ...
      <Text onClick={toggle} style={{ cursor: 'pointer' }}>
        {label}
      </Text>
      {tooltip ? (
