...
} from '../../components/common/list/listTypes';
import type {
  SortOption,
  SortOrder,
  SortValue,
...
// Sortable fields for Email Providers, mirroring the backend's whitelisted
// orderby columns (id, name, provider_type, sender_email, status, created_at,
// updated_at). `id` is the implicit default (newest first) and is intentionally
// not offered here.
export const SORT_OPTIONS: SortOption[] = [
  { value: 'name', label: __('Connection name', 'kelune-crm') },
  { value: 'provider_type', label: __('Type', 'kelune-crm') },
  { value: 'sender_email', label: __('Sender email', 'kelune-crm') },
...
// Default: `id` DESC (newest providers first). `id` is not a user-selectable
