...
// The wizard has four steps; reaching the last one marks the wizard finished.
const FINISH_STEP = 3;
...
/**
 * First-run setup wizard. Four steps: welcome + business identity, create lists,
 * create tags, and a congratulations screen. Style, layout and next/prev/skip
 * behaviour mirror the support-genix wizard. Progress (step + finished) persists
 * to the settings blob so the wizard resumes where it left off and never
 * re-opens once completed.
 */
const SetupWizard = () => {
  const dispatch = useDispatch();
  const { step: currentStep } = useSetupWizard();
  ...
  const storeSteps = useCallback(
    async (newStep: number, finished = false, running = true) => {
      const isFinished = finished || newStep === FINISH_STEP;
      setIsLoading(true);
  ...
  const stepItems = compactSteps
    ? [{ title: '' }, { title: '' }, { title: '' }, { title: '' }]
    : [
        { title: __('Welcome', 'kelune-crm') },
        { title: __('Lists', 'kelune-crm') },
        { title: __('Tags', 'kelune-crm') },
        { title: __('Finished', 'kelune-crm') },
      ];
