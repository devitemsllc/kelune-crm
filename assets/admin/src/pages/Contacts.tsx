...
const Contacts = () => {
  ...
  // Sync active tab with the router sub-route (/contacts/<tab>). Segments is a
  // Pro-only tab; fall back to the contacts list when Pro is inactive so a
  // typed /contacts/segments URL never lands on a missing tab.
  useEffect(() => {
    const sub = location.pathname.split('/')[2];
    if (sub === 'segments' && !isProActive()) {
      ...
      <Tabs activeKey={activeTab} tabBarStyle={{ marginBottom: 24 }}>
        ...
        >
          <ListPageHeader
            ...
            primaryAction={{
              label: __('Add Contact', 'kelune-crm'),
